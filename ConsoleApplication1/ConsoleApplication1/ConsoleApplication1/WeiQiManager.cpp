...
void WeiqiGameManager::doFinish(ActionInfo& actInfo) {
	...
	if (actInfo.winner == 0) {
		actInfo.strInfo = L"白棋:" + to_wstring(result[0]) + L"子  黑棋：" + to_wstring(result[1]) + L"子 白棋胜！";
	}
	else {
		actInfo.strInfo = L"黑棋:" + to_wstring(result[1]) + L"子  白棋：" + to_wstring(result[0]) + L"子 黑棋胜！";
	}
}
