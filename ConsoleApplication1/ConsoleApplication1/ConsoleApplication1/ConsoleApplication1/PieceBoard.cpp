...
bool PieceBoard::record_in(PieceColor color, PiecePos pos) {
	if (pos.x > 19 || pos.y > 19 || pos.x < 1 || pos.y < 1 || arr[pos.x - 1][pos.y - 1] != 1) {
		return false;
	}
	else {
		pos.x--;
...
void PieceBoard::reversePiece(PiecePos pos) {
	if (arr[pos.x - 1][pos.y - 1] == 2) {
		arr[pos.x - 1][pos.y - 1] = 3;
	}
	else if(arr[pos.x - 1][pos.y - 1] == 3){
		arr[pos.x - 1][pos.y - 1] = 2;
	}
}
